...
def test_service_token_prefers_dedicated_token():
    store.set_secret("yandex-mcp-metrika-token", "узкий")
    store.set_secret("yandex-mcp-token", "общий")
    assert tokens.service_token("metrika") == "узкий"


def test_service_token_falls_back_to_unified_login():
    store.set_secret("yandex-mcp-token", "общий")
    assert tokens.service_token("metrika") == "общий"


def test_service_token_error_mentions_login_and_storage():
    with pytest.raises(RuntimeError) as excinfo:
        tokens.service_token("metrika")
