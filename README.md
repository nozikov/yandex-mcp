...
yandex-mcp setup                # регистрация приложения Яндекса по шагам
yandex-mcp login                # вход: всё, что разрешено приложению
yandex-mcp status               # какие токены есть, где лежат, когда истекут
yandex-mcp logout               # удалить токены из хранилища
```

**Что именно доступно, решает панель Яндекса, а не настройки здесь.** `login` за одну
авторизацию просит все права — Метрику, Вебмастер и Директ — и сохраняет общий токен.
Включать и отключать доступы нужно в настройках приложения на
[oauth.yandex.ru](https://oauth.yandex.ru/), после чего достаточно повторить `login`.

Единственное, что не выдаётся по щелчку: право `direct:api` требует одобренной заявки
в кабинете Директа (до 7 дней). Пока её нет, Яндекс отклонит авторизацию с этим правом —
`login` заметит это, **сам войдёт без Директа** и скажет об этом. Метрика и Вебмастер
заработают сразу, а когда заявку одобрят, тот же `login` подхватит Директ.
...
Отключать отдельные инструменты в конкретном проекте удобнее на стороне клиента
(в Claude Code — управление инструментами MCP-сервера), а не конфигом сервера.
